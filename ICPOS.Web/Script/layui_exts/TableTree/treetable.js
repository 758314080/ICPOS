layui.define(["form"], function(exports) {
	...
	tree.prototype.template = function(data) {
		...
		$.each(data, function(index, item) {
			var hide_class = 'class="' + (item.Module_Parent == 0 || item.Module_Parent == t.cache(item.Module_Parent) || t.c.is_open ? "" : "hide") + '"',
				tr = '<tr data-id="' + item.Module_ID + '" data-pid="' + item.Module_Parent + '" ' + hide_class + ">" +
				(t.c.is_checkbox ? '<td><div><input type="checkbox" lay-skin="primary" lay-filter="lay-t"></div></td>' : "");
			level[item.Module_ID] = item.Module_Parent > 0 ? (level[item.Module_Parent] + 1) : 0;
			$.each(t.c.cols, function(idx, obj) {
				tr += '<td style="width:' + obj.width + '">';
				if(obj.field == t.c.field) {
					tr += ("&nbsp;".repeat(level[item.Module_ID] * t.c.space));
					if(t.c.childs[item.Module_ID]) {
						tr += '<i class="layui-icon ' + t.c.icon_class + '">' + (item.Module_ID == t.cache(item.Module_ID) || t.c.is_open ? t.c.icon_val.close : t.c.icon_val.open) + "</i>"
					}
				}
	...
	tree.prototype.render = function(e) {
		...
		$.each(t.c.data, function(index, item) {
			if(!t.c.childs[item.Module_Parent]) {
				t.c.childs[item.Module_Parent] = []
			}
			t.c.childs[item.Module_Parent][item.Module_ID] = t.c.new_data[item.Module_ID] = data[item.Module_ID] = item
		});
	...
	tree.prototype.tree = function(lists, pid, data) {
		...
		$.each(t.c.data, function(index, item) {
			if(item.Module_Parent == pid) {
				data.concat(t.tree(lists, item.Module_ID, data))
			}
		});
		return data
	};
